...
parser.add_argument('--face_detection_model', '-fd', type=str, default='C:/Users/marher/Documents/personal/development/text-to-speech/face_detection_yunet_2021dec.onnx', help='Path to the face detection model. Download the model at https://github.com/opencv/opencv_zoo/tree/master/models/face_detection_yunet')
parser.add_argument('--face_recognition_model', '-fr', type=str, default='C:/Users/marher/Documents/personal/development/text-to-speech/face_recognition_sface_2021dec.onnx', help='Path to the face recognition model. Download the model at https://github.com/opencv/opencv_zoo/tree/master/models/face_recognition_sface')
...
def welcome():
    winsound.PlaySound("welcome.mp3", winsound.SND_ASYNC | winsound.SND_ALIAS )

def bye():
    winsound.PlaySound("not.mp3", winsound.SND_ASYNC | winsound.SND_ALIAS )

def visualize(input, faces, fps, thickness=2, seen_flag=True):
    if faces[1] is not None:
        if seen_flag==False:
            # mixer.init()
            # mixer.music.load("hello.mp3")
            # mixer.music.play()
            # P = Process(target=welcome)
...
if __name__ == '__main__':
    ...
    if args.image1 is not None:
        ...
        visualize(img1, faces1, tm.getFPS(), thickness=2, seen_flag=True)
        ...
        if args.image2 is not None:
            ...
            visualize(img2, faces2, tm.getFPS(),thickness=2,seen_flag=True)
        ...
        while cv.waitKey(1) < 0:
            ...
            tm.stop()
            # Draw results on the input image
            seen_flag = visualize(frame, faces, tm.getFPS(),thickness=2,seen_flag=seen_flag)
